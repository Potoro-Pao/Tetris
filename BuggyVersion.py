...
def display_bricks(shape_location):
    ...
    for i in shape_location:
        green_brick[i] = (0, 200, 100)
        for brick_pos in green_brick:
            pygame.draw.rect(win, green_brick[brick_pos],
...
# produce the block
    ...
    green_brick = display_bricks(block_position)
    display_board(game_board)

# if you press arrow keys then the bricks won't move
    if event.type != pygame.KEYDOWN:
        if drop() == False:
            for (x, y) in green_brick:  # the dictionary will record the color of stone once the block reach bottom.
